...
class DjinniJobScraper:
    ...
    VACANCY_CART = "ul.list-jobs > li[id^='job-item']"
    ...
    def scrape_jobs(self, existing_links):
        ...
        for i, item in enumerate(vacancies):
            try:
                ...
                company_tag = item.select_one("a.text-body.js-analytics-event")
                company = company_tag.get_text(strip=True) if company_tag else "Unknown"

                info_spans = item.select_one("div.fw-medium span.text-nowrap, div.fw-medium span.location-text")
                info_items = [span.get_text(strip=True) for span in info_spans]

                self.logger.info(f"\n[{i+1}] {title} @ {company} info: {info_items}")
                self.logger.info(f"🔗 {full_link}")
                #is “You've applied”?
                applied_marker = item.find("a", string=lambda t: t and "applied" in t.lower())
                if applied_marker:
